...
def import_data(request):
    if request.method == "POST":
        ...
        file_path = base_url + (relative_path.replace('/', '\\'))
        ...
        messages.success(request, 'Your data is being imported, you will be notifield once it is done')
        return redirect('import_data')
    else:
...
def export_data(request):
    if request.method == "POST":
        ...
        # show the message to the user
        messages.success(request, 'Your data is being exported, you will be notifield once it is done')
